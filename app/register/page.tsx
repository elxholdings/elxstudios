...
export default async function RegisterPage({ searchParams }: { searchParams?: Promise<{ lang?: string | string[]; next?: string | string[] }> }) {
  ...
  return <SiteShell locale={locale}><main className="flex items-center px-5 py-8 md:px-10 lg:h-[calc(100svh-108px)] lg:min-h-[560px] lg:py-6"><AuthForm mode="register" next={next?.startsWith('/') ? next : '/dashboard'} /></main></SiteShell>;
}
