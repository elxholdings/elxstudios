...
import { FileStack, MiniChart, PlanSketch } from '../components/client-graphics';
...
export default async function PricingPage({ searchParams }: { searchParams?: Promise<{ lang?: string | string[] }> }) {
  ...
  const visuals = [
    <FileStack key="scope" className="h-12 w-16 text-[#102321]" />,
    <MiniChart key="complexity" className="h-12 w-16 text-[#102321]" />,
    <svg key="deadline" viewBox="0 0 64 48" className="h-12 w-16" fill="none" stroke="#102321" strokeWidth="2.5" strokeLinecap="square"><path d="M14 10h36v28H14z" /><path d="M22 6v8M42 6v8M14 20h36M22 29h6M35 29h7" /><path d="m47 34 7 7 8-14" stroke="#DDF65C" /></svg>,
    <PlanSketch key="revisions" className="h-12 w-16 text-[#102321]" />,
  ];
  return <SiteShell locale={locale}><main className="desktop-screen bg-[#F8F7F2] lg:h-[calc(100svh-108px)] lg:min-h-[620px]">
    ...
    <section className="px-5 py-8 md:px-10 lg:flex lg:h-[62%] lg:min-h-0 lg:items-center lg:py-6"><div className="mx-auto w-full max-w-[1440px]"><div className="grid gap-px bg-black/10 md:grid-cols-4">{factors.map(([title,text],index) => <article key={title} className="bg-white p-5 xl:p-6"><div className="flex items-start justify-between"><span className="text-xs font-black text-[#F06449]">0{index + 1}</span><span className="grid h-14 w-16 place-items-center bg-[#F5F2E8]">{visuals[index]}</span></div><h2 className="mt-5 text-2xl font-black tracking-[-.04em]">{title}</h2><p className="mt-3 text-sm leading-6 text-black/55">{text}</p></article>)}</div>
