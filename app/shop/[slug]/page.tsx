...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return product ? { title: product.title, description: product.summary, alternates: { canonical: `/shop/${product.slug}` }, openGraph: { title: product.title, description: product.summary, url: `/shop/${product.slug}`, type: 'website', images: product.cover_url ? [{ url: product.cover_url, alt: product.title }] : undefined } } : { title: 'Plan not found' };
}
...
export default async function ProductPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams?: Promise<{ lang?: string | string[] }> }) {
  ...
  const product = (await getPublishedProducts()).find((item) => item.slug === slug);
  if (!product) notFound();
  const productSchema = { '@context': 'https://schema.org', '@type': 'Product', name: product.title, description: product.summary, image: product.cover_url ? [product.cover_url] : undefined, category: product.category, brand: { '@type': 'Brand', name: 'Elx Studio' }, offers: { '@type': 'Offer', url: `https://elxholdings.com/shop/${product.slug}`, priceCurrency: product.currency, price: product.price, availability: 'https://schema.org/InStock', seller: { '@type': 'Organization', name: 'Elx Holdings' } } };
