...
const allowed = new Set(['image/jpeg','image/png','image/webp','image/gif','video/mp4','video/webm','video/quicktime','audio/mpeg','audio/mp3','audio/wav','audio/x-wav','audio/ogg','audio/mp4','audio/aac','audio/flac']);
...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File) || !allowed.has(file.type)) return NextResponse.json({ error: 'Upload a JPG, PNG, WebP, GIF, MP4, WebM, MOV, MP3, WAV, OGG, AAC or FLAC file.' }, { status: 400 });
