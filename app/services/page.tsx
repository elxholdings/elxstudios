...
export default async function ServicesPage({ searchParams }: { searchParams?: Promise<{ lang?: string | string[] }> }) {
  ...
  return <SiteShell locale={locale}>
    <main className="bg-[#F8F7F2] lg:h-[calc(100svh-108px)] lg:min-h-[620px]"><div className="lg:grid lg:h-full lg:grid-cols-[.72fr_1.28fr]"><ServiceProcess /><section className="px-5 py-10 md:px-10 lg:flex lg:min-h-0 lg:flex-col lg:px-[clamp(2rem,4vw,5rem)] lg:py-[clamp(1.75rem,4vh,3.5rem)]"><div className="mb-6 flex items-end justify-between gap-6"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#F06449]">Services list</p><h1 className="mt-2 text-4xl font-black tracking-[-.055em] xl:text-5xl">Choose the work.</h1></div><p className="hidden max-w-xs text-right text-xs leading-5 text-black/45 xl:block">Open any department for its deliverables, formats and turnaround.</p></div>
