...
export default async function ContactPage({ searchParams }: { searchParams?: Promise<{ lang?: string | string[] }> }) {
  ...
  const contactWhatsApp = getWhatsAppRoute(whatsappRouting, 'contact_page');
  return (
    <SiteShell locale={locale}>
      ...
      <section className="px-5 py-20 md:px-10 md:py-28">
        <div className="mx-auto grid max-w-[1100px] gap-5 md:grid-cols-2">
          ...
          <article className="bg-[#DDF65C] p-8 md:p-10"><p className="text-sm font-black uppercase tracking-[.16em]">Existing order or question</p><h2 className="mt-8 text-4xl font-black tracking-[-0.05em]">Continue on WhatsApp</h2><p className="mt-5 leading-7 text-black/60">Include your order reference when following up about a quote, progress update, delivery or revision.</p><a href={buildWhatsAppUrl(contactWhatsApp, {}, whatsappRouting.defaultCountryCode)} target="_blank" rel="noreferrer" className="mt-8 inline-flex bg-[#102321] px-5 py-4 text-sm font-black text-white">Open WhatsApp ↗</a></article>
        </div>
      </section>
    </SiteShell>
  );
